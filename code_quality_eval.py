...
def search(hash, type):
	...
	"""
	proc = subprocess.Popen('echo ' + hash + ' | ~/lookup/showCnt ' + type,
...
def calc_score(author):
	# getting the author's commits
	proc = subprocess.Popen('echo "'+ author + '" | ~/lookup/getValues a2c',
				stdout=subprocess.PIPE, shell=True)
	(out, err) = proc.communicate()
...
def find_links(author, end_time, method='sh'):
	...
	"""
	proc = subprocess.Popen('echo "'+ author + '" | ~/lookup/getValues a2p',
				stdout=subprocess.PIPE, shell=True)
	(out, err) = proc.communicate()
...
for author in authors:
	find_links(author, time.time())
	#calc_score_tree_comp(author)
